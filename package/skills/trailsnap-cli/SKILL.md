...
使用前可以通过 `trailsnap <command> -h` 或 `trailsnap <command> <subcommand> -h` 查看每个命令的详细帮助信息。通常情况下，你需要先根据用户的问题逐步筛选出检索条件（一个简单的[示例](examples/simple.md)），然后查询符合条件的照片列表。如果需要显示某张照片，你可以使用 `medias` 命令获取其URL地址。
